...
class LayananListScreen extends StatefulWidget {
  @override
  _LayananListScreenState createState() => _LayananListScreenState();

  const LayananListScreen({
    Key key,
    this.keyword,
    this.kategori,
  }) : super(key: key);
...
class _LayananListScreenState extends State<LayananListScreen> {
  ...
  void _getProyekApi() async {
    GeneralModel.checCk(
      ...
      await TabModel.proyekData(res, false).then((v) {
        if (v.error) {
            ...
            InkWell(
              ...
              child: IgnorePointer(
                child: SearcBox(
                  ...
                  textL: 45,
                  placeholder: "Cari Layanan yang anda inginkan",
                  eventtChange: (v) {
                    print(v);
